...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      ...
      languages: { en: `${base}/projects/${slug}`, ar: `${base}/ar/projects/${slug}` },
    },
    openGraph: {
      title: p.seoTitle,
      description: p.seoDescription,
...
export default async function ProjectPage({
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <SiteNav lang="en" altHref={`/ar/projects/${p.slug}`} altLabel="عربي" cvPath={CV_PATH} whatsapp={WHATSAPP} />

      <main id="main" className="mx-auto max-w-5xl px-5 py-10">
        <Reveal>
          <Link href="/#work" className="text-sm text-zinc-400 transition hover:text-white">← Back to all work</Link>
        </Reveal>
        ...
        <Reveal delay={140}>
          <div className="mt-6 flex flex-wrap gap-2">
            {p.live && <a href={p.live} target="_blank" className="btn-glow shine rounded-full bg-green-500 px-5 py-2.5 text-sm font-bold text-zinc-950 hover:bg-green-400">{p.liveLabel || "Live Demo"} ↗</a>}
            <a href={p.github} target="_blank" className="rounded-full border border-white/15 px-5 py-2.5 text-sm font-semibold transition hover:bg-white/10">GitHub Code ↗</a>
            <a href={WHATSAPP} target="_blank" className="rounded-full border border-white/15 px-5 py-2.5 text-sm font-semibold transition hover:bg-white/10">Want something like this? →</a>
          </div>
        </Reveal>
        ...
        <Reveal delay={100}>
          <div className="relative mt-8 overflow-hidden rounded-3xl border border-white/10 bg-zinc-900">
            ...
            {p.live && (
              <div className="absolute bottom-3 right-3 rounded-full bg-zinc-950/85 px-3 py-1.5 backdrop-blur">
        ...
        <Reveal>
          <div className="mt-8 rounded-3xl border border-white/10 bg-white/[0.03] p-6">
            ...
            <ul className="mt-4 grid gap-2 md:grid-cols-2">
              {p.features.map((f) => (
                <li key={f} className="rounded-xl bg-zinc-900 px-4 py-3 text-sm text-zinc-300 transition-colors hover:border-green-500/30">✓ {f}</li>
              ))}
            </ul>
          </div>
        </Reveal>
        ...
        <Reveal>
          <nav className="mt-10 grid gap-4 md:grid-cols-2" aria-label="More projects">
            {related.map((r, i) => (
              <Link key={r.slug} href={`/projects/${r.slug}`} className="card-lift group rounded-2xl border border-white/10 bg-white/[0.03] p-5 hover:border-green-500/40">
                <div className="text-[11px] uppercase tracking-widest text-zinc-500">{i === 0 ? "← Previous" : "Next →"}</div>
